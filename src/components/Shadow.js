class Shadow {
  ...
  retry(width, height, score) {
    //*initialization for game restart
    this.start = false;
    this.width = width;
    this.height = height;
    this.user = score;
  ...
  drawOpenShadow() {
    ...
    fill(255);
    text(
      "Press Space to Start\nUse up & down arrow to change gravity direction",
      this.width / 2,
      this.height / 2
    );
  }
  drawCloseShadow() {
    if (this.flag) {
      //* check the flag to update local storage only once
  ...
  updateLocalStorage() {
    //* store game result to local storage
  ...
  draw() {
    ...
    else if (document.getElementById("overPopup").className == "popup Show")
      this.drawCloseShadow(); //*show close shadow with popup after userfall
  }
